...
export function Contact() {
  ...
  return <section id="contact" className="section contact-page-section scroll-mt-20 bg-soft"><Container><SectionHeading eyebrow="Start a conversation" title="Contact MD Claims" description="Tell us a little about your practice and the support you are looking for." /><div className="mt-12 grid overflow-hidden rounded-3xl border border-border bg-white shadow-xl shadow-navy/5 lg:grid-cols-[.8fr_1.2fr]"><aside className="contact-panel"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10"><Building2 size={27} /></span><h3 className="mt-8 text-2xl font-extrabold">Let&apos;s discuss your practice needs.</h3><p className="mt-4 leading-7 text-slate-300">Contact details below are editable placeholders and should be replaced before launch.</p><dl className="mt-10 space-y-6">{[[Phone, "Phone", "[Add Phone Number]"], [Mail, "Email", "[Add Email Address]"], [MapPin, "Address", "[Add Business Address]"], [Clock3, "Business Hours", "[Add Business Hours]"]].map(([Icon, label, value]) => <div key={label as string} className="flex gap-4"><Icon className="mt-1 shrink-0 text-teal" size={20} /><div><dt className="text-sm text-slate-400">{label as string}</dt><dd className="mt-1 font-bold">{value as string}</dd></div></div>)}</dl></aside><form onSubmit={submit} className="grid gap-5 p-6 sm:grid-cols-2 sm:p-10" noValidate><Field label="Full Name" name="fullName" required /><Field label="Practice Name" name="practiceName" required /><Field label="Email Address" name="email" type="email" required /><Field label="Phone Number" name="phone" type="tel" /><Field label="NPI Number" name="npi" required placeholder="10-digit NPI" /><Field label="Medical Specialty" name="specialty" className="sm:col-span-2" /><label className="sm:col-span-2"><span className="field-label">Message <span aria-hidden="true">*</span></span><textarea name="message" required minLength={10} rows={5} className="input resize-y" placeholder="How can MD Claims support your practice?" /></label><div className="sm:col-span-2"><button type="submit" disabled={status.type === "loading"} className="button button-primary w-full justify-center sm:w-auto">{status.type === "loading" ? "Submitting..." : "Contact MD Claims ->"}</button>{status.message && <p role="status" className={`mt-4 text-sm font-bold ${status.type === "success" ? "text-teal-dark" : "text-red-700"}`}>{status.message}</p>}</div></form></div></Container></section>;
}
